...
try {
const { rows } = await pool.query(
`SELECT DISTINCT s.id, s.title, s.author, s.bpm, s.key_signature, s.pinned, s.created_at, s.updated_at FROM songs s LEFT JOIN lyrics_blocks lb ON lb.song_id = s.id WHERE s.title ILIKE $1 OR s.author ILIKE $1 OR lb.content ILIKE $1 ORDER BY s.title LIMIT 30`,
[`%${q}%`]
);
// Attach categories
