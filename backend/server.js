...
const { name, pin, color } = req.body;
if (pin && !/^\d{4}$/.test(String(pin))) return res.status(400).json({ error: ‘PIN 4 chiffres requis’ });
if (pin) {
...
const { rows } = await pool.query(‘INSERT INTO categories(name,color) VALUES($1,$2) RETURNING *’, [name.toUpperCase(), color||’#8b1a2e’]);
res.json(rows[0]);
});
...
if(n === normInput) return true;
if(normInput.length >= 4 && n.includes(normInput)) return true;
if(n.length >= 4 && normInput.includes(n)) return true;
const wa = normInput.split(’ ‘).filter(w=>w.length>2);
const wb = n.split(’ ’).filter(w=>w.length>2);
if(!wa.length || !wb.length) return false;
const shared = wa.filter(w=>wb.includes(w)).length;
return shared / Math.max(wa.length, wb.length) >= 0.6;
});
...
const shared = wa.filter(w=>wb.includes(w)).length;
const similarity = Math.round(shared / Math.max(wa.length, wb.length) * 100);
...
song ? res.json(song) : res.status(404).json({ error: ‘Not found’ });
});
...
console.error(’DB error attempt ’ + (i+1) + ’: ’ + err.message);
if (i < retries - 1) {
var wait = (i + 1) * 3000;
...
console.error(‘Starting without DB - check DATABASE_URL’);
app.listen(PORT, function() { console.log(’Server started (no DB) on port ’ + PORT); });
}
